...
Windows 本地优先、离线可用的个人工作台。当前 1.1 版本包含加密本地数据、任务列表/GTD/四象限、Markdown 笔记、提示词模板、番茄钟、本地日历、习惯打卡、喝水提醒、受控文件索引、今日总览、周报导出、托盘快捷入口与加密备份恢复。
...
- 应用保持离线，不包含云同步、邮件、外部日历、RSS 或 GitHub 集成。
- 文件索引必须由用户在应用内手动选择文件夹；仅索引元数据，不读取或上传文件内容。
